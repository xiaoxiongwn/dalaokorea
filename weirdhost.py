...
STATUS_SELECTOR = 'p[class*="StatusText"]'
...
class WeirdHostRenewal:
    ...
    def get_status_text(self, sb, timeout=30):
        """状态元素可能比页面晚加载，因此显式等待并提供页面文本兜底。"""
        try:
            sb.wait_for_element_visible(STATUS_SELECTOR, timeout=timeout)
            element = sb.find_element(STATUS_SELECTOR)
            text = element.text.strip()
            if text:
                return text
        except Exception as e:
            self.log(f"⚠️ 未找到 StatusText 状态元素：{e}")

        try:
            return sb.get_text("body").strip()
        except Exception:
            return ""

    def renewal_available(self, sb):
        status_text = self.get_status_text(sb)
        self.log(f"📋 当前状态：{status_text[:300]}")

        if "지금 연장이 가능해요" in status_text:
            return True

        # 兜底：直接检查续期按钮是否存在且可见。
        try:
            sb.wait_for_element_visible(RENEW_BUTTON, timeout=5)
            return True
        except Exception:
            return False
        ...
        with SB(
            ...
            try:
                ...
                for attempt in range(1, 4):
                    ...
                    status_text = self.get_status_text(sb, timeout=15)
                    can_renew = "지금 연장이 가능해요" in status_text

                    # 点击续期后，按钮通常应不再处于“可立即续期”状态。
                    if final_timestamp != "未找到" and not can_renew:
                        confirmed = True
                        break

                    self.log(
                        "⚠️ 页面结果尚未明确刷新，"
                        f"当前到期时间：{final_timestamp}，继续等待..."
                    )
                    time.sleep(10)
                ...
                if confirmed:
                    ...
                    msg = (
                        "⚠️ WeirdHost 已执行续期操作，但暂未能明确确认页面状态\n"
